...
_setup_squashfs_overlayfs() {
    ...
    for i in "${overlay}" "${static}" "${work}"; do
        [ ! -d "${i}" ] && mkdir -p "${i}"
    done
    ...
    mount -t squashfs -o loop,ro "${LOOP_PATH}" "${static}"
    mount -t tmpfs none "${overlay}"
    mount -t overlay overlay -olowerdir=${static},upperdir=${overlay},\
	    workdir=${work} ${NEW_ROOT}

    [ ! -d "${NEW_ROOT}${overlay}" ] && mkdir -p "${NEW_ROOT}${overlay}"
    [ ! -d "${NEW_ROOT}${static}" ] && mkdir -p "${NEW_ROOT}${static}"
    [ ! -d "${NEW_ROOT}${work}" ] && mkdir -p "${NEW_ROOT}${work}"
...
_livecd_mount_squashfs() {
    ...
    # if OVERLAYFS, redirect to the squashfs+aufs setup function
    if _is_overlayfs; then
        good_msg "Mounting squashfs & overlayfs filesystems"
        _setup_squashfs_overlayfs
        test_success "Mount filesystem"
        return  # preserve old behaviour
    fi
